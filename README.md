...
## 1. 개요
CreativeSeoul은 디자인, 마케팅, 비주얼 분야에 특화된 구인구직 플랫폼으로, 한국과 해외의 디자인 감각 차이에 주목해 개발되었습니다. 글로벌 시장과 한국 시장을 연결하는 크리에이티브 인재와 기업을 위한 백엔드 애플리케이션입니다.

## 2. 배포 및 API 명세
- **[CreativeSeoul](https://creativeseoul.com)**
- **[Swagger API 명세](https://api.creativeseoul.com/api)**
...
### Redis와 백그라운드 작업을 활용한 채용 지원 클릭 트래킹

**배경**  
채용 공고를 게시한 기업 회원에게 지표를 제공하기 위해 채용 지원 클릭 수 트래킹 기능 필요.

**문제점**  
매 클릭마다 데이터베이스를 직접 업데이트하는 방식은 트랜잭션 오버헤드 발생 및 응답 지연 문제 야기.

**해결 방법**
- Redis 메모리 기반 데이터 스토어 활용한 클릭 데이터 캐싱 구현
- String 타입과 TTL 기능을 활용한 IP 기반 중복 클릭 방지 메커니즘
- Hash 타입 활용으로 다수 채용 공고의 클릭 데이터 효율적 관리
- BullMq를 활용한 백그라운드 작업으로 주기적 데이터 동기화 구현 (5분 간격)
- CASE 문을 활용한 대량 업데이트 쿼리로 DB 부하 최소화

📝 [전체 글 읽기](https://medium.com/@oink2716/redis와-백그라운드-작업을-활용한-채용-지원-클릭-트래킹-시스템-구현-0e3021c39030)
